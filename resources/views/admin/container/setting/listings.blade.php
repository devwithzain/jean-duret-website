...
<div class="p-5">
    ...
    <form action={{ route('admin.container.setting.update',$user->id) }} method="post" class="w-full flex flex-col gap-5 py-8"
       ...
       <div class="flex flex-col gap-1.5">
        ...
        <input id="image" name="image" accept="image/*" onchange="previewImage(event)"
           class="w-full outline-none focus:border-black/50 px-3 py-1.5 text-black text-md bg-white font-normal rounded-lg border border-black/10 shadow-[0_0_1px_rgba(0,0,0,0.2)]"
           type="file">
        <img id="imagePreview" src="{{ $user->image ? asset('storage/' . $user->image) : asset('assets/placeholder.png') }}" alt="Preview" class="mt-2 w-40 rounded-lg">
     </div>
